           ...
           self.append(msg)
             ...
             for m in msg:
                if type(m) not in (str,int,float):
                    print("don't know how to encode message element " + str(m) + " " + str(type(m)))
                    return
                self.append(m)
        else:
            print("don't know how to encode message " + str(m) + " " + str(type(m)))
            return
...
class CallbackManager:
    ...
    def handle(self, data, source, disable_osc):
        """Given OSC data, tries to call the callback with the right address."""
        decoded = decodeOSC(data)
        self.dispatch(decoded, source, disable_osc)

    def dispatch(self, message, source, disable_osc):
        """Sends decoded OSC data to an appropriate calback"""
        address = message[0]

#        if address.startswith("/util/") == False and disable_osc == True:
#            return

        group_add_found = False
        for key, value in self.group_callbacks.items():
            if address.startswith(key):

                self.group_callbacks[key](message, source)
                group_add_found = True

        if group_add_found == False:
            for key, value in self.callbacks.items():
                if key == address:
                    self.callbacks[address](message, source)


    def group_add(self, address, callback):
        if callback == None:
            del self.group_callbacks[address]
        else:
            self.group_callbacks[address] = callback
